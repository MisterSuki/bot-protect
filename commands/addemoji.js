...
if(!nome) return message.channel.send("**<:aloi_aviso:777754787855859714> | (emoji) você esqueceu de especificar um nome**")

} else {
return message.channel.send("**<:aloi_aviso:777754787855859714> | Você precisa especificar um (link) um (emoji) uma (imagem) ou (random)**");
}
} else if (link && !message.attachments.first() && isEmoji(args[0]).response === false) {
link = args[0]
nome = args.slice(1).join("")
if(!nome) return message.channel.send("**<:aloi_aviso:777754787855859714> | (link) você esqueceu de especificar um nome**")

const a = new Discord.MessageAttachment(link,  "foto.gif")
message.channel.send(a).then((s) => {
if(s.attachments.first().size >= 256000) {
message.channel.send("**<:aloi_aviso:777754787855859714> | Este (link) pesa `"+converter(s.attachments.first().size)+"` e necessário ser menor que \`256KB\`**")
}
